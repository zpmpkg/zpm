...
class SATSolver {
    ...
    async load() {
        ...
        if (util_1.isDefined(this.lockContent)) {
            const types = this.registries.getTypes();
            this.assumptions = [];
            for (const type of types) {
                try {
                    for (var _b = __asyncValues(axioms_1.get(this.lockContent.named, [type], [])), _c; _c = await _b.next(), !_c.done;) {
                        const pkg = _c.value;
                        const found = await this.registries.searchPackage(type, { name: pkg.name });
                        if (util_1.isDefined(found)) {
                            this.assumptions.push(this.toTerm(found.getHash(), new version_1.Version(pkg.version)));
                        }
                        else {
                            // @todo
                        }
    ...
    async addDefinition(hash, definition, parent) {
        ...
        await async_1.settledPromiseAll([
            ...lodash_1.flatten(lodash_1.toPairs(definition.packages.named).map(p => p[1]
                .filter(x => package_1.isGitPackageEntry(x))
                .map((e) => ({
                description: e,
            ...
            ...lodash_1.flatten(lodash_1.toPairs(definition.packages.path).map(p => p[1]
                .filter(x => package_1.isPathPackageEntry(x))
                .map((e) => ({
                description: {
                    name: e.name || '$ROOT',
                    version: e.version,
